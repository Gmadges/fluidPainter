...
{                             
    ...
    vec2 vT = texture2D(Velocity, coord + vec2(0.0, delta.y)).rg;
    vec2 vB = texture2D(Velocity, coord - vec2(0.0, delta.y)).rg;      
    vec2 vR = texture2D(Velocity, coord + vec2(delta.x, 0.0)).rg;       
    vec2 vL = texture2D(Velocity, coord - vec2(delta.x, 0.0)).rg;              
